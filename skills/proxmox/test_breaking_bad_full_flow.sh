...
echo
echo "== Request Breaking Bad S1 (TPB should have it) =="
...
code=$(curl -s -o /tmp/jelly_bb.out -w "%{http_code}" -b /tmp/jelly.cookie -X POST "$JELLY/api/v1/request" -H 'Content-Type: application/json' --data-binary '{"mediaType":"tv","mediaId":1396,"seasons":[1],"is4k":false}')
echo "HTTP $code"
head -c 300 /tmp/jelly_bb.out; echo

sleep 5

echo
echo "== Trigger Sonarr search for Breaking Bad S1 =="
python3 -c "
import json, urllib.request
s = json.load(urllib.request.urlopen('http://192.168.11.66:8989/api/v3/series/lookup?term=81189&apikey=0375672f0f64474c8843b922d0ab595e'))
if s and len(s) > 0:
    sid = s[0].get('id')
    if sid:
        ...
        with urllib.request.urlopen(req) as r:
            result = json.load(r)
            print(f'Search command sent: {result.get(\"name\")}')
"

sleep 20

echo
echo "== Check Sonarr queue (should have releases from TPB) =="
python3 -c "
import json, urllib.request
...
for r in q.get('records',[])[:10]:
    print(f'  - {r.get(\"series\",{}).get(\"title\")} S{r.get(\"episode\",{}).get(\"seasonNumber\")}E{r.get(\"episode\",{}).get(\"episodeNumber\")}: {r.get(\"status\")} | {r.get(\"size\")/(1024*1024):.1f} MB | indexer: {r.get(\"indexer\")}')
"
...
ls -lh /root/arr-stack/media/downloads/complete/ 2>/dev/null | tail -10 || echo "No files yet"
ls -lh /root/arr-stack/media/tv/ 2>/dev/null | tail -10 || echo "TV dir empty"
